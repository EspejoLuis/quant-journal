...
7. **README.md** — check if any new files, notes, or products were added that should be reflected in the tables (Models, Numerical Methods, Products, Notes). Also check if all links in `notes` folder are present in the README.md file.
   ...
   - Status audit: for every row in the tables, set status to Done only when ALL of these are true: (1) .hpp, .cpp, and tests columns all have links, AND (2) no unchecked items in project_progress.md are specifically about that feature's implementation. Verification steps (e.g. "build and run tests") and items for unrelated features (e.g. "PDE engine") do NOT block Done for a different feature — filter to only the items that belong to the feature being audited. If the only unchecked items are verification steps or items for other features, set to Done. If either condition is unmet for feature-specific items, keep it as In progress if work on that has started. If no work has started then Planned is fine.
