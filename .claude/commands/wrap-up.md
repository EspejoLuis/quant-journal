...
2. **Anki screenshot** — find the most recent screenshot in the `log` folder or Desktop/Downloads newer than yesterday's log. Rename it to `Anki_<D><Mon>_<YYYY>.<ext>`, move it to `log/<YYYY>/<MM-Mon>/images/`. Update the image link in today's log.
...
6. **README.md** — check if any new files, notes, or products were added that should be reflected in the tables (Models, Numerical Methods, Products, Notes).
